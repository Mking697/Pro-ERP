...
export async function deleteOrganization(orgId: string): Promise<void> {
  ...
  await db.batch([
    ...
    db.delete(creditNoteUsages).where(eq(creditNoteUsages.orgId, orgId)),
    db.delete(creditNotes).where(eq(creditNotes.orgId, orgId)),
    db.delete(debitNoteUsages).where(eq(debitNoteUsages.orgId, orgId)),
    db.delete(debitNotes).where(eq(debitNotes.orgId, orgId)),
    db.delete(expenseEntries).where(eq(expenseEntries.orgId, orgId)),
    db.delete(pettyCashEntries).where(eq(pettyCashEntries.orgId, orgId)),
