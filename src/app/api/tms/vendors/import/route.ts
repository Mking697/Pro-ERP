...
import {
  createTransportVendorsBulk,
  type BulkCreateTransportVendorRowInput,
} from "@/lib/tms/transportVendors";
...
type VendorField = keyof Omit<BulkCreateTransportVendorRowInput, "row">;
...
export async function POST(request: Request) {
  ...
  let parsed;
  try {
    parsed = await parseImportFile<VendorField, BulkCreateTransportVendorRowInput>(
      buffer,
      file.name,
      file.type,
      HEADER_ALIASES,
  ...
  const result = await createTransportVendorsBulk(parsed.rows, guard.session.email);
